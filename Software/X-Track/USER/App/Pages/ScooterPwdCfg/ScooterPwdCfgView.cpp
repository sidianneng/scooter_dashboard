...
void ScooterPwdCfgView::Create(lv_obj_t* root)
{
    ...
    ui.curpwd_textarea = pwd_textarea_Create(cont, 120, 20, "cur pwd");
    ui.newpwd_textarea = pwd_textarea_Create(cont, 120, 50, "new pwd");
    ui.cfmpwd_textarea = pwd_textarea_Create(cont, 120, 80, "cfm pwd");
    ui.setpwd_button = btn_Create(cont, "OK", 0, 120);
}

lv_obj_t* ScooterPwdCfgView::pwd_textarea_Create(lv_obj_t* root, uint8_t x_oft, uint8_t y_oft,\
    const char* textarea_name)
{
    ...
    lv_textarea_set_text(pwd_textarea, "123");
...
lv_obj_t* ScooterPwdCfgView::btn_Create(lv_obj_t* root, const char* btn_name, uint8_t x_oft, \
    uint8_t y_oft)
{
    lv_obj_t* cont = lv_obj_create(root);
    lv_obj_remove_style_all(cont);
